...
public class PositionZPanel extends JPanel {
	
	private static final long serialVersionUID = 1L;
	private final JLabel currentPositionYlabel = new JLabel("a");
	...
	Thread monitorLeap = new Thread() {
		public void run()
		{
			...
			while(monitor)
			{
				...
				{
					getOrgans();
//					System.out.println(mainFinger != null);
					if(mainHand != null && mainFinger != null)
					{
						
						if(list2.getSelectedValue().toString().equals("Palm"))
						{
//							System.out.println("monitor z still on");
							int tempZ = (int)mainHand.palmPosition().getZ();
//							System.out.println(tempZ);
							if(tempZ != previousZ && tempZ > rangeSlider.getValue() && tempZ < rangeSlider.getUpperValue())
							{
//								lblO.setLocation(80, (int)(tempZ*0.1)+150);
								lblO.setLocation(80, map(tempZ, rangeSlider.getValue(), rangeSlider.getUpperValue(), 115, 5));
    							currentPositionYlabel.setText(Integer.toString(((int)tempZ)));
    							previousZ = tempZ;
							}    							
						}
						else
						{
							int tempZ = (int)mainFinger.tipPosition().getZ();
							if(tempZ != previousZ && tempZ > rangeSlider.getValue() && tempZ < rangeSlider.getUpperValue())
							{
								lblO.setLocation(80, map(tempZ, rangeSlider.getValue(), rangeSlider.getUpperValue(), 115, 5));
    							currentPositionYlabel.setText(Integer.toString(((int)tempZ)));
			...
			System.out.println("leaving monitor leap thread position Z");
		}
...
//    								lblO.setLocation(80, map(tempZ, rangeSlider.getValue(), rangeSlider.getUpperValue(), 115, 5));
				...
				if(parent.btnTestOutput.isEnabled())
				{
					...
					if(isQualified())
					{
						...
						{
							if(previousZ > Integer.parseInt(rangeSliderValue1.getText().toString()) 
									&& previousZ < Integer.parseInt(rangeSliderValue2.getText().toString())
									&& preprevious != previousZ)
							{
								int mapped = map(previousZ, Integer.parseInt(rangeSliderValue1.getText().toString()),
...
//							System.out.println("note onnnn");
							...
							else if((previousZ > Integer.parseInt(rangeSliderValue1.getText().toString()) 
									&& previousZ < Integer.parseInt(rangeSliderValue2.getText().toString()))
									&& !noteOff)
							{
								try {
...
//		refreshLists();
    ...
    public PositionZPanel(MappingProperties mp)
	{
